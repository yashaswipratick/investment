...
public class StockAnalyserDataHttpEntryLoader {
    ...
    private static final String LOADER_TAG = "[ANALYSER_LOADER]";
    ...
    /**
     * Returns true for errors that should be retried (up to 3 attempts):
     *   - HTTP 401 / 403  — session cookie expired; a fresh cookie will be generated on retry
     *   - AsyncRequestTimeoutException, TimeoutException, and any Netty/Reactor timeout
     *     These are transient and typically resolve on a subsequent attempt.
     */
    private static boolean isRetryableError(Throwable t) {
        ...
        // Covers: AsyncRequestTimeoutException, ReadTimeoutException, WriteTimeoutException,
        //         reactor.netty timeouts, and any other class whose simple name contains "Timeout"
    ...
    private Mono<StockHistory> fetchApiData(WebClient client, String url, String stockSymbol) {
        return client.get()
                ...
                .map(StockAnalyserDataHttpEntryLoader::decodeResponse)
                .map(String::new)
                ...
                .doOnNext(sh -> log.info("Fetched Stock History NSE details for {}", stockSymbol))
                        ...
                        retrySignals
                                .flatMap(signal -> {
                                    ...
                                    if (isRetryableError(failure)) {
                                        log.warn("Retrying SAME URL for {} (attempt #{}) | url={} | {} — {}",
                                                stockSymbol, signal.totalRetries() + 1, url,
                                                failure.getClass().getSimpleName(), failure.getMessage());
                                        return Mono.just(signal);
                                    }
                                    return Mono.error(failure); // non-retryable — propagate immediately
                                })
                                .delayElements(Duration.ofSeconds(5)) // longer delay for timeouts
                                .take(3)
                ))
                .doOnError(e -> log.error("NSE fetchApiData failed for {} after retries | url={} | {}", stockSymbol, url, e.getMessage()))
    ...
    private Mono<List<StockHistoryDetails>> fetchApiDataList(WebClient client, String url) {
        return client.get()
                ...
                .map(String::new)
                .map(response -> {
                    List<StockHistoryDetails> details = StockAnalyserDataHttpEntryLoader.convertResponseToDto(response);
                ...
                .retryWhen(Retry.from(retrySignals ->
                        retrySignals
                                .flatMap(signal -> {
                                    Throwable failure = signal.failure();
                                    if (isRetryableError(failure)) {
                                        log.warn("Retrying SAME URL on fetchApiDataList (attempt #{}) | url={} | {} — {}",
                ...
                .doOnError(e -> log.error("NSE fetchApiDataList failed after retries | url={} | {}", url, e.getMessage()))
                .onErrorResume(error -> Mono.empty());
    }
