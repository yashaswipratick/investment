...
public class NiftyFiftyStockInfoHttpEntryLoader {
    ...
    public NiftyFiftyStockInfoHttpEntryLoader(
    ...
    public Mono<StockInfoDetails> getStockDetails(String symbol) {
        ...
        return nseSessionManager.generateFreshSessionCookie()
                ...
                .flatMap(client ->
                        fetchApiData(client, buildURL(symbol)))
                .doOnError(error ->
                        log.error(
                                "symbol -> {}, Error: {}",
                                symbol,
    ...
    private Mono<StockInfoDetails> fetchApiData(
        ...
        return client.get()
                ...
                .map(NiftyFiftyStockInfoHttpEntryLoader::decodeResponse)
                .map(String::new)

                .doOnNext(response ->
                        log.debug("NSE Response => {}", response))

                .map(NiftyFiftyStockInfoHttpEntryLoader::convertResponseToDto)

                .filter(dto -> dto != null)
                .filter(dto -> dto.getInfo() != null)
                ...
                .doOnNext(details ->
                        log.info(
                                "Fetched NSE stock details for {}",
                                details.getStockInfo()
                                        .keySet()))
                ...
                .doOnError(error ->
                        log.error(
                                "Failed to fetch NSE data. URL: {}, Error: {}",
                                url,
                                error.getMessage()));
    }
